...
async function getQuestDataTableRawTsv() {
  ...
  try {
    ...
    if (response.status === 304 && cache?.tsv) {
      logger.log("FairQuestRewards quest table unchanged in gdocs, using cache");
      return cache.tsv;
    }
    ...
    logger.log("Loaded FairQuestRewards quest table from gdocs");
    return tsv;
  } catch (error) {
    if (cache?.tsv) {
      logger.warn(`Failed to fetch FairQuestRewards quest table from gdocs, using cache: ${(error as Error).message}`);
      return cache.tsv;
    }
    throw new Error(`Failed to fetch FairQuestRewards quest table from gdocs and no cache is available: ${(error as Error).message}`);
  }
}
