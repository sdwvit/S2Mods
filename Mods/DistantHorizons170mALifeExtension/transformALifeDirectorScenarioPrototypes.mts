...
import { SQUARED_FACTOR } from "./transformAIGlobals.mts";
import type { StructTransformer } from "../../src/meta-type.mts";

/**
 * Transforms ALifeDirectorScenarioPrototypes to adjust NPC limits and spawn parameters.
 */
export const transformALifeDirectorScenarioPrototypes: StructTransformer<
  ALifeDirectorScenarioPrototype
> = async (struct, {}) => {
  ...
  const ALifeScenarioNPCArchetypesLimitsPerPlayerRank =
    struct.ALifeScenarioNPCArchetypesLimitsPerPlayerRank.map(([_k, e], i) => {
      if (!i) {
        return;
      }
      const fork = e.fork();
  ...
  const ScenarioGroups = struct.ScenarioGroups.map(([_, v]) => {
    ...
    if (v.PostSpawnDirectorTimeoutMin)
      fork.PostSpawnDirectorTimeoutMin = Math.ceil(v.PostSpawnDirectorTimeoutMin / SQUARED_FACTOR);
    if (v.PostSpawnDirectorTimeoutMax)
      fork.PostSpawnDirectorTimeoutMax = Math.ceil(v.PostSpawnDirectorTimeoutMax / SQUARED_FACTOR);
  ...
  Object.assign(newStruct, {
    ...
    ScenarioGroups,
    DefaultALifeLairExpansionToPlayerTimeMax: Math.ceil(
      struct.DefaultALifeLairExpansionToPlayerTimeMax / SQUARED_FACTOR,
    ),
    DefaultALifeLairExpansionToPlayerTimeMin: Math.ceil(
      struct.DefaultALifeLairExpansionToPlayerTimeMin / SQUARED_FACTOR,
    ),
