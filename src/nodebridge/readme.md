...
Windows proxy DLL that loads into the Stalker 2 process (takes the `dwmapi.dll`
slot), spawns a bundled `node.exe` as a child, and routes RPC between the game
and per-mod JS entry points over a named pipe.

This directory holds the native side. The JS runtime bundle, mod scaffolding,
and install scripts live in the wider repo (`src/pull-node-runtime.mts`,
`src/inject-nodebridge.mts`, `Mods/NodeBridge/`).
...
- `src/` — DLL source (C++20).
  ...
  - `dwmapi.def` — forwarder exports pointing at `C:\Windows\System32\dwmapi.*`.
