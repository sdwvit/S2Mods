...
export interface GameApi {
    ...
    getObjectCount(): Promise<{
        count: number;
    } | Unresolved>;
    listObjects(opts?: ListObjectsOptions): Promise<ListObjectsResult | Unresolved>;
    getObjectByIndex(index: number): Promise<ObjectRef | {
        found: false;
        index: number;
    } | Unresolved>;
    getObjectByName(name: string): Promise<ObjectRef | {
        found: false;
        name: string;
    } | Unresolved>;
    getPlayerPawn(): Promise<{
        found: true;
        index: number;
        name: string;
        className: string;
    ...
    getPlayerLocation(): Promise<Vector3 & {
        rootOffset: number;
        locOffset: number;
    } | Unresolved>;
    setPlayerLocation(value: Vector3): Promise<{
        ok: boolean;
        reason?: string;
        rootOffset: number;
        locOffset: number;
    ...
    getProperty(target: number | string, prop: string): Promise<{
        ok: true;
        offset: number;
    } | {
        ok: false;
    ...
    listProperties(target: number, max?: number): Promise<{
        target: number;
        count: number;
        properties: Array<{
            name: string;
    ...
    dumpClassMemory(target: number, offset: number, count?: number): Promise<{
        target: number;
        offset: number;
        count: number;
        classPtr: number;
    ...
    dumpObjectMemory(target: number, offset: number, count?: number): Promise<{
        target: number;
        offset: number;
        count: number;
        objPtr: number;
    ...
    readMemory(addr: number, count?: number): Promise<{
        addr: number;
        count: number;
        hex: string;
    } | {
    ...
    writeMemory(addr: number, hex: string): Promise<{
        addr: number;
        count: number;
    } | {
        addr: number;
    ...
    virtualFree(addr: number): Promise<{
        ok: true;
        addr: number;
    } | {
        ok: false;
    ...
    virtualProtect(addr: number, size: number, protect: number): Promise<{
        ok: true;
        addr: number;
        size: number;
        protect: number;
    ...
    scanAOB(pattern: string): Promise<{
        pattern: string;
        hit: number;
    } | {
        error: string;
    ...
    fnameToString(comp: number, num?: number): Promise<{
        comp: number;
        num: number;
        name: string;
    } | Unresolved>;
    ...
    fnameFromString(str: string): Promise<{
        comp: number;
        num: number;
    } | {
        error: string;
